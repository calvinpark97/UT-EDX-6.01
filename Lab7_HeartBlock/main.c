...
int main(void){
  ...
  while(1){           // Follows the nine steps list above
    SetReady();				//a) Make the Ready Green Led signal go high
    WaitForASLow();		//b) Wait for AS to be 0, SW1 to be pressed
    ClearReady();			//c) Make the Ready Green Led signal go low
    Delay1ms(10);			//d) Wait 10ms
    WaitForASHigh();	//e) Wait for AS to be 1, SW1 to be released
    Delay1ms(250);		//f) Wait 250ms
    SetVT();					//g) Make the VT RED Led signal go high
    Delay1ms(250);		//h) Wait 250ms
    ClearVT();				//i) Make the VT Red Led signal go low
  }
}
...
void WaitForASLow(void){
	...
	while(SW1 != 0){									//Loop continues until SW1 = 0 which is pressed
...
void WaitForASHigh(void){
	...
	while(SW1 == 0){								//Loop continues until SW1 = 1 which is released
		SW1 = GPIO_PORTF_DATA_R & 0x10;
	}
	}
